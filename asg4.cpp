...
int main(){
    ...
    ifstream MyFile("RandomNormal1D.txt");
    for(i=0;i<d;i++){
    ...
    writecountfile.open("4quicknormalcount.txy",fstream::app);
    writecountfile<<k[j]<<'\n';
    writecountfile.close();
    ofstream writefile("4Aftersortquicknormal.txy");
    for(i=0;i<d;i++){
